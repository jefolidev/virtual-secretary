  ...
  async execute({
    ...
    try {
      ...
      if (!tokenRecord || !tokenRecord.syncToken) {
        console.warn(
          `[PushNotif] Channel not found or missing syncToken for channelId=${channelId}`,
        )
      ...
      console.log(
        `[PushNotif] Changes received: ${changes.changes.length}`,
      ...
      for (const change of changes.changes) {
        console.log(
          `[PushNotif] Processing change: googleEventId=${change.googleEventId} status=${change.status} start=${change.startDateTime}`,
        )
        ...
        if (!calendarEvent) {
          console.log(
            `[PushNotif] No calendarEvent found for googleEventId=${change.googleEventId} — skipping`,
          )
          continue
        }
        ...
        if (!appointment) {
          console.log(
            `[PushNotif] No appointment found for calendarEvent appointmentId=${calendarEvent.appointmentId}`,
          )
          continue
        }
        ...
        if (change.status === 'cancelled') {
          if (!appointment.isCancelled()) {
            appointment.cancel('google-calendar')
            await this.appointmentsRepository.save(appointment)
            console.log(
              `[PushNotif] Appointment ${appointment.id} cancelled from Google Calendar`,
            )
          }
          continue
        }
        ...
        if (change.startDateTime && change.endDateTime) {
          ...
          console.log(
            `[PushNotif] Time comparison — effectiveStart=${effectiveStart.toISOString()} newStart=${newStart.toISOString()}`,
          )
          ...
          console.log(
            `[PushNotif] startChanged=${startChanged} endChanged=${endChanged}`,
          )
          ...
          if (startChanged || endChanged) {
            ...
            console.log(
              `[PushNotif] Appointment ${appointment.id} updated to start=${newStart.toISOString()}`,
            )

            const professionalUser =
              await this.userRepository.findByProfessionalId(
      ...
      console.error(
        `[HandleCalendarPushNotification] Unhandled error for channelId=${channelId}:`,
        errorMessage,
        error instanceof Error ? error.stack : '',
      )
      return left(`Unhandled error: ${errorMessage}`)
    }
  }
}
