...
export type MarkAppointmentAsNoShowUseCaseResponse = Either<
...
export class MarkAppointmentAsNoShowUseCase {
  constructor(
  ...
  async execute({
    appointmentId,
    professionalId,
  }: MarkAppointmentAsNoShowUseCaseRequest) {
    ...
    if (!appointment.professionalId.equals(new UniqueEntityId(professionalId)))
      return left(
        new NotAllowedError(
          'You cannot mark appointment as no show if that is not yours.'
        )
      )
    ...
    if (appointment.status !== 'SCHEDULED')
      return left(
        new BadRequestError(
          'You cannot cannot mark appointment as no show if is not scheduled.'
        )
      )
