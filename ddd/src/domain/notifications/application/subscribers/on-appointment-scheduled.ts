  ...
  private async sendNewAppointmentNotification({
    ...
    if (professional && client) {
      await this.sendNotification.execute({
        ...
        content: `O paciente ${client.name.toUpperCase()} agendou uma consulta para ${dayjs(appointment.startDateTime).format('DD/MM/YYYY')} às ${dayjs(appointment.startDateTime).format('hh:mm')}.`,
      })
    }
  }
}
